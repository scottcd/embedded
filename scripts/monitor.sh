...
usage() {
    cat <<'EOF'
Usage: ./scripts/monitor.sh [target] [options]

Open an ESP-IDF serial monitor.
...
Options:
  ...
  -B, --baud <baudrate>    Monitor baud rate
  -h, --help               Show this help text

Any additional arguments are forwarded to idf.py before the monitor command.

Examples:
  ./scripts/monitor.sh
  ./scripts/monitor.sh reference_app
  ./scripts/monitor.sh reference_app --port /dev/ttyACM1
  ./scripts/monitor.sh reference_app --baud 115200
...
cmd+=("${idf_args[@]}" monitor)

echo "Monitoring ${target} via ${port}"
exec "${cmd[@]}"
